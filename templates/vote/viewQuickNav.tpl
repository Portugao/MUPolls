{* purpose of this template: votes view filter form *}
{checkpermissionblock component='MUPolls:Vote:' instance='::' level='ACCESS_EDIT'}
{assign var='objectType' value='vote'}
<form action="{$modvars.ZConfig.entrypoint|default:'index.php'}" method="get" id="mUPollsVoteQuickNavForm" class="mupolls-quicknav">
    <fieldset>
        ...
        <input type="hidden" name="ot" value="vote" />
        ...
        {/if}
        <input type="submit" name="updateview" id="mupolls_votequicknav_updateview" value="{gt text='OK'}" />
    </fieldset>
</form>
<script type="text/javascript">
/* <![CDATA[ */
    document.observe('dom:loaded', function() {
        mUMUPollsInitQuickNavigation('vote');
        {{if isset($searchFilter) && $searchFilter eq false}}
            {{* we can hide the submit button if we have no quick search field *}}
            $('mupolls_votequicknav_updateview').addClassName('z-hide');
        {{/if}}
    });
/* ]]> */
</script>
