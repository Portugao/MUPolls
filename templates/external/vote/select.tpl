...
<p>
    <label for="{$baseID}Id"{$leftSide}>{gt text='Vote'}:</label>
    <select id="{$baseID}Id" name="id"{$rightSide}>
        {foreach item='vote' from=$items}
            <option value="{$vote.id}"{if $selectedId eq $vote.id} selected="selected"{/if}>{$vote->getTitleFromDisplayPattern()}</option>
        {foreachelse}
