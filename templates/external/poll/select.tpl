...
<p>
    <label for="{$baseID}Id"{$leftSide}>{gt text='Poll'}:</label>
    <select id="{$baseID}Id" name="id"{$rightSide}>
        {foreach item='poll' from=$items}
            <option value="{$poll.id}"{if $selectedId eq $poll.id} selected="selected"{/if}>{$poll->getTitleFromDisplayPattern()}</option>
        {foreachelse}
