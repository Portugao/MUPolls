...
<p>
    <label for="{$baseID}Id"{$leftSide}>{gt text='Option'}:</label>
    <select id="{$baseID}Id" name="id"{$rightSide}>
        {foreach item='option' from=$items}
            <option value="{$option.id}"{if $selectedId eq $option.id} selected="selected"{/if}>{$option->getTitleFromDisplayPattern()}</option>
        {foreachelse}
...
<p>
    ...
    <input type="button" id="mUPollsSearchGo" name="gosearch" value="{gt text='Filter'}" />
    <br{$break} />
</p>
<br />
<br />
...
/* <![CDATA[ */
    document.observe('dom:loaded', function() {
        mUPolls.itemSelector.onLoad('{{$baseID}}', {{$selectedId|default:0}});
    });
/* ]]> */
</script>
