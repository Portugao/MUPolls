...
<div class="row">
    <div class="col-sm-8">
        <div class="form-group">
            <label for="{$baseID}Id" class="col-sm-3 control-label">{gt text='Option'}:</label>
            <div class="col-sm-9">
                <select id="{$baseID}Id" name="id" class="form-control">
                    {foreach item='option' from=$items}
                        <option value="{$option->getKey()}"{if $selectedId eq $option->getKey()} selected="selected"{/if}>{$option->getTitle()}</option>
                    {foreachelse}
            ...
            <div class="col-sm-9">
                <div class="input-group">
                    ...
                    <span class="input-group-btn">
                        <input type="button" id="mUPollsModuleSearchGo" name="gosearch" value="{gt text='Filter'}" class="btn btn-default" />
                    </span>
                </div>
            </div>
        </div>
