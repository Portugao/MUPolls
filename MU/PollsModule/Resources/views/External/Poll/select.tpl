...
<div class="row">
    <div class="col-sm-8">
        <div class="form-group">
            <label for="{$baseID}Id" class="col-sm-3 control-label">{gt text='Poll'}:</label>
            <div class="col-sm-9">
                <select id="{$baseID}Id" name="id" class="form-control">
                    {foreach item='poll' from=$items}
                        <option value="{$poll->getKey()}"{if $selectedId eq $poll->getKey()} selected="selected"{/if}>{$poll->getTitle()}</option>
                    {foreachelse}
