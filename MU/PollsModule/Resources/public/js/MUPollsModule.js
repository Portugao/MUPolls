...
function mUPollsInitQuickNavigation() {
    ...
    if (jQuery('.mupollsmodule-quicknav').length < 1) {
        return;
    }

    quickNavForm = jQuery('.mupollsmodule-quicknav').first();
    objectType = quickNavForm.attr('id').replace('mUPollsModule', '').replace('QuickNavForm', '');

    quickNavForm.find('select').change(function (event) {
        quickNavForm.submit();
    });

    var fieldPrefix = 'mupollsmodule_' + objectType.toLowerCase() + 'quicknav_';
...
jQuery(document).ready(function () {
    ...
    isViewPage = jQuery('.mupollsmodule-view').length > 0;
    isDisplayPage = jQuery('.mupollsmodule-display').length > 0;
