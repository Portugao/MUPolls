 ...
 */
function mUPollsCreateInlineEditingWindowInstance(containerElem)
{
    var newWindowId;
 ...
 */
function mUPollsInitInlineEditingWindow(objectType, idPrefix, containerId, inputType)
{
    ...
    jQuery.each(mUPollsInlineEditHandlers, function (key, editHandler) {
        ...
        editHandler.windowInstanceId = mUPollsCreateInlineEditingWindowInstance(jQuery('#' + containerId));
    });
    ...
    newEditHandler = {
        alias: idPrefix,
        prefix: containerId,
        moduleName: 'MUPollsModule',
        objectType: objectType,
        inputType: inputType,
        windowInstanceId: mUPollsCreateInlineEditingWindowInstance(jQuery('#' + containerId))
    };

    // add it to the list of edit handlers
    mUPollsInlineEditHandlers.push(newEditHandler);
}

/**
 * Creates a link for editing an existing item using inline editing.
 */
function mUPollsCreateInlineEditLink(objectType, idPrefix, elemPrefix, itemId)
{
 ...
 */
function mUPollsInitInlineEditLink(objectType, idPrefix, elemPrefix, itemId, inputType)
{
    jQuery('#' + elemPrefix + 'Edit').click(function (event) {
        event.preventDefault();
        mUPollsInitInlineEditingWindow(objectType, idPrefix, idPrefix + 'Reference_' + itemId + 'Edit');
    });
}
 ...
 */
function mUPollsDetermineInputReference(objectType, alias, idPrefix, inputType, targetWindow)
{
    var inputPrefix, inputIdentifier, inputField;

    // determine reference to input element
    inputPrefix = targetWindow.jQuery('.mupolls-edit-form').first().attr('name');
    inputField = null;
 ...
 */
function mUPollsInitInlineEditingButtons(objectType, alias, idPrefix, inputType, createUrl)
{
    var inputReference, createButtonId, createButton, itemIds, itemIdsArr;

    inputReference = mUPollsDetermineInputReference(objectType, alias, idPrefix, inputType, window);
    ...
    if (jQuery('#' + createButtonId).length < 1) {
        ...
        createButton = jQuery('<a />', {
            ...
            class: 'btn btn-default mupollsmodule-inline-button'
        }).append(
    ...
    createButton.click(function (event) {
        event.preventDefault();
        mUPollsInitInlineEditingWindow(objectType, idPrefix, createButtonId, inputType);
    });
    ...
    jQuery.each(itemIdsArr, function (key, existingId) {
        ...
        if (existingId) {
            ...
            jQuery('#' + elemPrefix).click(function (event) {
                event.preventDefault();
                mUPollsInitInlineEditingWindow(objectType, idPrefix, elemPrefix, inputType);
            });
        }
    });
}
 ...
 */
function mUPollsCloseWindowFromInside(idPrefix, itemId, formattedTitle, searchTerm)
{
    ...
    jQuery.each(window.parent.mUPollsInlineEditHandlers, function (key, editHandler) {
        ...
        inputReference = mUPollsDetermineInputReference(editHandler.objectType, editHandler.alias, idPrefix, inputType, window.parent);
        if (null === inputReference || null === inputReference.field) {
            return;
        }
        ...
        window.parent.mUPollsSimpleAlert(anchorElement, window.parent.Translator.__('Information'), window.parent.Translator.__('Action has been completed.'), 'actionDoneAlert', 'success');
