...
abstract class AbstractPollsModuleInstaller extends AbstractExtensionInstaller
{
    ...
    public function install()
    {
        ...
        try {
            ...
            $logger->error('{app}: Could not create the database tables during installation. Error details: {errorMessage}.', ['app' => 'MUPollsModule', 'errorMessage' => $e->getMessage()]);
        ...
        switch ($oldVersion) {
            case '1.0.0':
                ...
                try {
                    ...
                    $this->addFlash('error', $this->__('Doctrine Exception') . ': ' . $e->getMessage());
                    $logger->error('{app}: Could not update the database tables during the upgrade. Error details: {errorMessage}.', ['app' => 'MUPollsModule', 'errorMessage' => $e->getMessage()]);
    ...
    */
    ...
    protected function updateExtensionInfoFor14()
    {
        ...
        $conn->executeQuery("
            ...
            SET name = 'MUPollsModule',
                directory = 'MU/PollsModule'
            WHERE name = 'Polls';
        ");
    }
    ...
    public function uninstall()
    {
        ...
        $result = Zikula_Workflow_Util::deleteWorkflowsForModule('MUPollsModule');
        if (false === $result) {
            $this->addFlash('error', $this->__f('An error was encountered while removing stored object workflows for the %extension% extension.', ['%extension%' => 'MUPollsModule']));
            $logger->error('{app}: Could not remove stored object workflows during uninstallation.', ['app' => 'MUPollsModule']);
        ...
        try {
            ...
            $this->addFlash('error', $this->__('Doctrine Exception') . ': ' . $e->getMessage());
            $logger->error('{app}: Could not remove the database tables during uninstallation. Error details: {errorMessage}.', ['app' => 'MUPollsModule', 'errorMessage' => $e->getMessage()]);
    ...
    /**
     * Create the default data for MUPollsModule.
     *
     * @return void
     */
