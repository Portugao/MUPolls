...
abstract class AbstractPollController extends AbstractController
{
     ...
     * @ParamConverter("poll", class="MUPollsModule:PollEntity", options = {"id" = "id", "repository_method" = "selectById"})
    ...
    protected function displayInternal(Request $request, PollEntity $poll, $isAdmin = false)
    {
        ...
        $instanceId = $poll->createCompositeIdentifier();
    ...
    protected function deleteInternal(Request $request, PollEntity $poll, $isAdmin = false)
    {
        ...
        $logArgs = ['app' => 'MUPollsModule', 'user' => $this->get('zikula_users_module.current_user')->get('uname'), 'entity' => 'poll', 'id' => $poll->createCompositeIdentifier()];
        ...
        $form = $this->createForm('MU\PollsModule\Form\DeleteEntityType', $poll);
        
        if ($form->handleRequest($request)->isValid()) {
            if ($form->get('delete')->isClicked()) {
                $hookHelper = $this->get('mu_polls_module.hook_helper');
                // Let any hooks perform additional validation actions
                $validationHooksPassed = $hookHelper->callValidationHooks($poll, 'validate_delete');
                if ($validationHooksPassed) {
                    ...
                    // Let any hooks know that we have deleted the poll
                    $hookHelper->callProcessHooks($poll, 'process_delete', null);
    ...
    protected function handleSelectedEntriesActionInternal(Request $request, $isAdmin = false)
    {
        ...
        foreach ($items as $itemId) {
            ...
            try {
                ...
                $logger->error('{app}: User {user} tried to execute the {action} workflow action for the {entity} with id {id}, but failed. Error details: {errorMessage}.', ['app' => 'MUPollsModule', 'user' => $userName, 'action' => $action, 'entity' => 'poll', 'id' => $itemId, 'errorMessage' => $e->getMessage()]);
            }
            ...
            if ($action != 'delete') {
                ...
                $url = new RouteUrl('mupollsmodule_poll_' . /*($isAdmin ? 'admin' : '') . */'display', $urlArgs);
            }
