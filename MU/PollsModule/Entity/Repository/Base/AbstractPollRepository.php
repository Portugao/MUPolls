 ...
 *
 * This is the base repository class for poll entities.
 */
abstract class AbstractPollRepository extends EntityRepository
{
    ...
    protected function getViewQuickNavParameters($context = '', $args = [])
    {
        ...
        $parameters['multiple'] = $this->getRequest()->query->get('multiple', '');
        $parameters['inFrontend'] = $this->getRequest()->query->get('inFrontend', '');
    ...
    public function truncateTable(LoggerInterface $logger)
    {
        ...
        $qb->delete('MU\PollsModule\Entity\PollEntity', 'tbl');
        $query = $qb->getQuery();
    ...
    public function updateCreator($userId, $newUserId, TranslatorInterface $translator, LoggerInterface $logger, CurrentUserApi $currentUserApi)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->update('MU\PollsModule\Entity\PollEntity', 'tbl')
        ...
        $logArgs = ['app' => 'MUPollsModule', 'user' => $currentUserApi->get('uname'), 'entities' => 'polls', 'userid' => $userId];
     ...
     *
     * @return array|pollEntity retrieved data array or pollEntity instance
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
     ...
     * @param boolean $slimMode If activated only some basic fields are selected without using any joins (optional) (default=false)
     *
     * @return ArrayCollection collection containing retrieved pollEntity instances
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
    ...
    public function addCommonViewFilters(QueryBuilder $qb)
    {
        ...
        foreach ($parameters as $k => $v) {
            ...
            } elseif (in_array($k, ['multiple', 'inFrontend'])) {
                // boolean filter
                if ($v == 'no') {
                    $qb->andWhere('tbl.' . $k . ' = 0');
                } elseif ($v == 'yes' || $v == '1') {
    ...
    {
        $startDate = null !== $this->getRequest() ? $this->getRequest()->query->get('dateOfStart', date('Y-m-d H:i:s')) : date('Y-m-d H:i:s');
        $qb->andWhere('(tbl.dateOfStart <= :startDate OR tbl.dateOfStart IS NULL)')
           ->setParameter('startDate', $startDate);
        $endDate = null !== $this->getRequest() ? $this->getRequest()->query->get('dateOfEnd', date('Y-m-d H:i:s')) : date('Y-m-d H:i:s');
    ...
    {
        $selection = 'COUNT(tbl.id) AS numPolls';
        if (true === $useJoins) {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->select($selection)
           ->from('MU\PollsModule\Entity\PollEntity', 'tbl');
     ...
     * @param integer $excludeId  Id of polls to exclude (optional)
     *
    ...
    protected function genericBaseQueryAddWhere(QueryBuilder $qb, $where = '')
    {
        if (!empty($where) || null !== $this->getRequest()) {
            ...
            $filterPluginManager = new FilterPluginManager(
                ...
                [
                    new DateFilter(['dateOfStart', 'dateOfEnd'/*, 'tblJoin.someJoinedField'*/])
                ],
    ...
    public function getQueryFromBuilder(QueryBuilder $qb)
    {
        ...
        $featureActivationHelper = \ServiceUtil::get('mu_polls_module.feature_activation_helper');
        if ($featureActivationHelper->isEnabled(FeatureActivationHelper::TRANSLATIONS, 'poll')) {
