...
namespace MU\PollsModule\Form\Handler\Option\Base;

use MU\PollsModule\Form\Handler\Common\EditHandler;
use MU\PollsModule\Form\Type\OptionType;
...
abstract class AbstractEditHandler extends EditHandler
{
    ...
    public function processForm(array $templateParameters = [])
    {
        ...
        $this->objectTypeCapital = 'Option';
        $this->objectTypeLower = 'option';
        ...
        if ($this->templateParameters['mode'] == 'create') {
            if (!$this->modelHelper->canBeCreated($this->objectType)) {
                $this->request->getSession()->getFlashBag()->add('error', $this->__('Sorry, but you can not create the option yet as other items are required which must be created before!'));
                $logArgs = ['app' => 'MUPollsModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => $this->objectType];
    ...
    {
        return $this->formFactory->create(OptionType::class, $this->entityRef, $this->getFormOptions());
    }
    ...
    protected function getRedirectCodes()
    {
        ...
        // user index page of option area
        $codes[] = 'userIndex';
        // admin index page of option area
        $codes[] = 'adminIndex';
        // user list of options
        $codes[] = 'userView';
        // admin list of options
        $codes[] = 'adminView';
        // user list of own options
        $codes[] = 'userOwnView';
        // admin list of own options
        $codes[] = 'adminOwnView';
        // user detail page of treated option
        $codes[] = 'userDisplay';
        // admin detail page of treated option
        $codes[] = 'adminDisplay';
    ...
    protected function getDefaultReturnUrl(array $args = [])
    {
        ...
        $routePrefix = 'mupollsmodule_' . $this->objectTypeLower . '_' . $routeArea;
    ...
    protected function getDefaultMessage(array $args = [], $success = false)
    {
        ...
        switch ($args['commandName']) {
            case 'submit':
                if ($this->templateParameters['mode'] == 'create') {
                    $message = $this->__('Done! Option created.');
                } else {
                    $message = $this->__('Done! Option updated.');
                }
                break;
            case 'delete':
                $message = $this->__('Done! Option deleted.');
                break;
            default:
                $message = $this->__('Done! Option updated.');
                break;
    ...
    public function applyAction(array $args = [])
    {
        ...
        try {
            ...
            $logArgs = ['app' => 'MUPollsModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => 'option', 'id' => $entity->getKey(), 'errorMessage' => $exception->getMessage()];
    ...
    protected function getRedirectUrl(array $args = [])
    {
        ...
        if ($this->request->getSession()->has('mupollsmodule' . $this->objectTypeCapital . 'Referer')) {
            $this->request->getSession()->remove('mupollsmodule' . $this->objectTypeCapital . 'Referer');
        }
