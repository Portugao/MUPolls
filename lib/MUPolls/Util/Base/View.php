 ...
 */
class MUPolls_Util_Base_View extends Zikula_AbstractBase
{
    /**
    ...
    {
        $extensions = array();
        $hasAdminAccess = SecurityUtil::checkPermission('MUPolls:' . ucfirst($type) . ':', '::', ACCESS_ADMIN);
        if ($func == 'view') {
            if ($hasAdminAccess) {
                $extensions = array('csv', 'rss', 'atom', 'xml', 'json', 'kml');
            } else {
                $extensions = array('rss', 'atom');
            }
        } elseif ($func == 'display') {
            if ($hasAdminAccess) {
                $extensions = array('xml', 'json', 'kml', 'ics');
            } else {
