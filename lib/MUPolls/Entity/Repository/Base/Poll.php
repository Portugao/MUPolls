 ...
 */
class MUPolls_Entity_Repository_Base_Poll extends EntityRepository
{
    /**
    ...
    public function getAdditionalTemplateParameters($context = '', $args = array())
    {
        ...
        if ($context == 'controllerAction') {
            ...
            if (in_array($args['action'], array('main', 'view'))) {
                ...
                $templateParameters['workflowStateItems'] = $listHelper->getEntries('poll', 'workflowState');
                $booleanSelectorItems = array(
                    array('value' => 'no', 'text' => __('No')),
                    array('value' => 'yes', 'text' => __('Yes'))
                );
                $templateParameters['multipleItems'] = $booleanSelectorItems;
            }
        }
    ...
    protected function getViewQuickNavParameters($context = '', $args = array())
    {
        ...
        $parameters['multiple'] = isset($this->controllerArguments['multiple']) ? $this->controllerArguments['multiple'] : FormUtil::getPassedValue('multiple', '', 'GET');
    ...
    public function truncateTable()
    {
        ...
        $qb->delete('MUPolls_Entity_Poll', 'tbl');
        $query = $qb->getQuery();
    ...
    public function updateCreator($userId, $newUserId)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->update('MUPolls_Entity_Poll', 'tbl')
    ...
    {
        $results = $this->selectByIdList(array($id), $useJoins, $slimMode);
    ...
    public function getSelectWherePaginatedQuery(QueryBuilder $qb, $currentPage = 1, $resultsPerPage = 25)
    {
        ...
        $paginatorClass = 'MUPolls_Paginator_' . (!$isLegacy ? 'Paginator' : 'LegacyPaginator');
    ...
    public function selectWherePaginated($where = '', $orderBy = '', $currentPage = 1, $resultsPerPage = 25, $useJoins = true, $slimMode = false)
    {
        ...
        if (!$hasFilters) {
            if ($page > 1 || isset($_GET['pos'])) {
                ...
                SessionUtil::setVar('MUPollsPollsCurrentPage', $page);
            } else {
                // restore current page from session
                $page = SessionUtil::getVar('MUPollsPollsCurrentPage', 1);
    ...
    public function addCommonViewFilters(QueryBuilder $qb)
    {
        ...
        foreach ($parameters as $k => $v) {
            if ($k == 'catId') {
                 ...
                 */
                $qb = ModUtil::apiFunc('MUPolls', 'category', 'buildFilterClauses', array('qb' => $qb, 'ot' => 'poll', 'catids' => $v));
            } elseif (in_array($k, array('q', 'searchterm'))) {
                // quick search
                if (!empty($v)) {
                    $qb = $this->addSearchFilter($qb, $v);
                }
            } elseif (in_array($k, array('multiple'))) {
                // boolean filter
                if ($v == 'no') {
                    $qb->andWhere('tbl.' . $k . ' = 0');
                } elseif ($v == 'yes' || $v == '1') {
    ...
    protected function addSearchFilter(QueryBuilder $qb, $fragment = '')
    {
        ...
        if (!$fragmentIsNumeric) {
            ...
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.dateOfStart = \'' . $fragment . '\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.dateOfEnd = \'' . $fragment . '\'';
        } else {
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.title LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
    ...
    public function retrieveCollectionResult(Query $query, $orderBy = '', $isPaginated = false)
    {
        ...
        if ($isLegacy) {
            ...
            if (!$isPaginated) {
                ...
                $paginatorClass = 'MUPolls_Paginator_Paginator';
                $hasRelationships = false;
                $paginator = new $paginatorClass($query, $hasRelationships);
     ...
     * @param int    $excludeId  Id of polls to exclude (optional).
     *
    ...
    protected function genericBaseQueryAddWhere(QueryBuilder $qb, $where = '')
    {
        ...
        $showOnlyOwnEntries = (int) FormUtil::getPassedValue('own', ModUtil::getVar('MUPolls', 'showOnlyOwnEntries', 0), 'GETPOST');
