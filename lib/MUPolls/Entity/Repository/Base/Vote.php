 ...
 *
 * This is the base repository class for vote entities.
 */
class MUPolls_Entity_Repository_Base_Vote extends EntityRepository
{
    /**
    ...
    public function getAdditionalTemplateParameters($context = '', $args = array())
    {
        ...
        if ($context == 'controllerAction') {
            ...
            if (in_array($args['action'], array('main', 'view'))) {
                ...
                $templateParameters['workflowStateItems'] = $listHelper->getEntries('vote', 'workflowState');
            }
        }
    ...
    public function truncateTable()
    {
        ...
        $qb->delete('MUPolls_Entity_Vote', 'tbl');
        $query = $qb->getQuery();
    ...
    public function updateCreator($userId, $newUserId)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->update('MUPolls_Entity_Vote', 'tbl')
     ...
     *
     * @return array|voteEntity retrieved data array or voteEntity instance
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
     ...
     * @param boolean $slimMode If activated only some basic fields are selected without using any joins (optional) (default=false).
     *
     * @return ArrayCollection collection containing retrieved voteEntity instances
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
    ...
    public function selectWherePaginated($where = '', $orderBy = '', $currentPage = 1, $resultsPerPage = 25, $useJoins = true, $slimMode = false)
    {
        ...
        if (!$hasFilters) {
            if ($page > 1 || isset($_GET['pos'])) {
                ...
                SessionUtil::setVar('MUPollsVotesCurrentPage', $page);
            } else {
                // restore current page from session
                $page = SessionUtil::getVar('MUPollsVotesCurrentPage', 1);
    ...
    public function addCommonViewFilters(QueryBuilder $qb)
    {
        ...
        foreach ($parameters as $k => $v) {
            if ($k == 'catId') {
                 ...
                 */
                $qb = ModUtil::apiFunc('MUPolls', 'category', 'buildFilterClauses', array('qb' => $qb, 'ot' => 'vote', 'catids' => $v));
    ...
    protected function getCountQuery($where = '', $useJoins = true)
    {
        ...
        $selection = 'COUNT(tbl.id) AS numVotes';
        if ($useJoins === true) {
     ...
     * @param int    $excludeId  Id of votes to exclude (optional).
     *
     * @return boolean result of this check, true if the given vote does not already exist
     */
