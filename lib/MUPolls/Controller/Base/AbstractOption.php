 ...
 */
abstract class MUPolls_Controller_Base_AbstractOption extends Zikula_AbstractController
{
    /**
     * Post initialise.
     *
    ...
    public function main()
    {
        ...
        $objectType = 'option';
        $utilArgs = array('controller' => 'option', 'action' => 'index');
        ...
        if ($legacyControllerType == 'admin') {
            ...
            $redirectUrl = ModUtil::url($this->name, 'option', 'view', array('lct' => $legacyControllerType));
        ...
        $this->view->setCacheId('option_main');
        
        // return main template
        return $this->view->fetch('option/main.tpl');
    }
    /**
    ...
    public function view()
    {
        ...
        // parameter specifying which type of objects we are treating
        $objectType = 'option';
        $utilArgs = array('controller' => 'option', 'action' => 'view');
        ...
        $currentUrlObject = new Zikula_ModUrl($this->name, 'option', 'view', ZLanguage::getLanguageCode(), $currentUrlArgs);
    ...
    public function display()
    {
        ...
        // parameter specifying which type of objects we are treating
        $objectType = 'option';
        $utilArgs = array('controller' => 'option', 'action' => 'display');
        $permLevel = $legacyControllerType == 'admin' ? ACCESS_ADMIN : ACCESS_READ;
        $this->throwForbiddenUnless(SecurityUtil::checkPermission($this->name . ':' . ucfirst($objectType) . ':', '::', $permLevel), LogUtil::getErrorMsgPermission());
        $entityClass = $this->name . '_Entity_' . ucfirst($objectType);
        $repository = $this->entityManager->getRepository($entityClass);
        ...
        $currentUrlObject = new Zikula_ModUrl($this->name, 'option', 'display', ZLanguage::getLanguageCode(), $currentUrlArgs);
    ...
    public function edit()
    {
        ...
        // parameter specifying which type of objects we are treating
        $objectType = 'option';
        $utilArgs = array('controller' => 'option', 'action' => 'edit');
        ...
        $handlerClass = $this->name . '_Form_Handler_Option_Edit';
    ...
    public function delete()
    {
        ...
        // parameter specifying which type of objects we are treating
        $objectType = 'option';
        $utilArgs = array('controller' => 'option', 'action' => 'delete');
        ...
        $workflowHelper = new MUPolls_Util_Workflow($this->serviceManager);
        ...
        if (!$deleteAllowed) {
            return LogUtil::registerError($this->__('Error! It is not allowed to delete this option.'));
        }
        ...
        if ($confirmation) {
            ...
            $hookHelper = new MUPolls_Util_Hook($this->serviceManager);
            ...
            if ($validationHooksPassed) {
                ...
                // Let any hooks know that we have deleted the option
                $hookType = 'process_delete';
                $hookHelper->callProcessHooks($entity, $hookType, null);
                
                // The option was deleted, so we clear all cached pages this item.
    ...
    public function adminHandleSelectedEntries()
    {
        ...
        foreach ($items as $itemid) {
            ...
            if ($action != 'delete') {
                ...
                $url = new Zikula_ModUrl($this->name, 'option', 'display', ZLanguage::getLanguageCode(), $urlArgs);
            }
        ...
        $redirectUrl = ModUtil::url($this->name, 'admin', 'main', array('ot' => 'option'));
